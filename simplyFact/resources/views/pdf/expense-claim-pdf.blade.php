...
<body class="p-8 max-w-4xl mx-auto text-sm">
  ...
  <header class="flex items-start justify-between mb-8 pb-6 border-b-2 border-gray-200">
    ...
    <div class="text-right">
      ...
      @if(!empty($expensesClaim->committee_name))
        <p class="text-gray-500 text-xs mt-1">{{ $expensesClaim->committee_name }}</p>
      @endif
    </div>
  </header>
  ...
  <section class="grid grid-cols-2 gap-6 mb-8">
    <div class="bg-gray-50 rounded-xl p-5">
      ...
      @if(!empty($user->address_street))
        ...
        @if(!empty($user->address_country) && $user->address_country !== 'France')
    ...
    <div class="bg-gray-50 rounded-xl p-5">
      ...
      <p class="text-gray-500 text-xs mt-1">Commission : {{ $expensesClaim->committee_name ?? '' }}</p>
      ...
      @if(!empty($vehicle))
        <div class="mt-3 pt-3 border-t border-gray-200">
          ...
          <p class="font-medium text-gray-900">
            ...
            {{ !empty($vehicle->electrical) ? '(electrique)' : '' }}
            - {{ $vehicle->number_plate ?? '' }}
          </p>
          @if(!empty($vehicle->power))
  ...
  <section class="mb-6">
    <h2 class="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">Déplacements kilométriques</h2>
    <div class="rounded-xl overflow-hidden border border-gray-200">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          @php
              $rateFFS = match(true) {
                  in_array($trip->trip_type, ['Covoiturage, remorque, salarié...', 'Déplacements pendant stage fédéral']) => 0.40,
                  $trip->trip_type === 'voiture' => 0.36,
                  default => 0.14,
              };
          ...
          <tr class="bg-white text-center">
              ...
              <td class="px-4 py-3 text-gray-900">
                  {{ number_format($trip->total_distance ?? 0, 0, ',', ' ') }} km
              </td>
          </tr>
          @endforeach
        </tbody>
        <tfoot class="bg-gray-800 text-white text-xs">
          <tr class="font-bold text-center">
            ...
            <td class="px-4 py-3">
              {{ number_format($drivenTrips->sum('total_distance') ?? 0, 0, ',', ' ') }} km
            </td>
          </tr>
        </tfoot>
      </table>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="bg-white">
            ...
            <td class="px-4 py-3 text-right text-gray-600">{{ $acc->nb_of_night ?? 0}}</td>
            <td class="px-4 py-3 text-right text-gray-600">
              {{ number_format($acc->total_price ?? 0, 2, ',', ' ') }} €
            </td>
            <td class="px-4 py-3 text-right font-semibold text-gray-900">
              {{ number_format($acc->reimbursed_price ?? 0, 2, ',', ' ') }} €
            </td>
          </tr>
          @endforeach
        </tbody>
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="bg-white">
            <td class="px-4 py-3 text-gray-900">
              ...
              @if(!empty($expense->nb_days_of_training))
  ...
  <section class="mt-2">
    <div class="rounded-xl border-2 border-gray-200 overflow-hidden">
      <div class="divide-y divide-gray-100">
        ...
        @if(($computed->totalGiven ?? 0) > 0)
          @if(($computed->abandonKm ?? 0) > 0)
        ...
        <div class="flex justify-between px-6 py-3 text-sm bg-gray-100">
          ...
          <span class="font-bold text-lg text-gray-900">{{ number_format($computed->totalNDF ?? 0, 2, ',', ' ') }} €</span>
        </div>
        ...
        <div class="flex justify-between px-6 py-4 bg-gray-900">
          ...
          <span class="font-bold text-xl text-green-400">
            {{ number_format($expensesClaim->total_reimbursed ?? $computed->netTotal ?? 0, 2, ',', ' ') }} €
          </span>
        </div>
  ...
  <section class="mt-4 bg-green-50 border border-green-200 rounded-xl px-5 py-4 text-xs text-green-800">
    ...
    <p class="mt-1">
      ...
      <strong>{{ number_format($computed->taxReduction ?? 0, 0, ',', ' ') }} €</strong>
    </p>
  </section>
  @endif
  ...
  <section class="mt-4 border-2 border-gray-200 rounded-xl px-5 py-4 text-xs text-gray-600">
    ...
    <div class="flex justify-between py-1 border-b border-gray-100">
      ...
      <span class="font-medium text-gray-900">{{ number_format($computed->fullSettlementTotal ?? 0, 2, ',', ' ') }} €</span>
    </div>
    <div class="flex justify-between py-1">
      <span>Estimation réduction d'impôt (hors frais réels) si 100% en abandon</span>
      <span class="font-medium text-gray-900">{{ number_format($computed->fullAbandonTotal ?? 0, 0, ',', ' ') }} €</span>
    </div>
  </section>
